"use client"

import {
	Root as SwitchPrimitiveRoot,
	Thumb as SwitchPrimitiveThumb,
} from "@radix-ui/react-switch"
import { Label } from "#library/label/label"
import { XStack } from "#primitives/x-stack"
import { cn } from "#utils/cn"

interface ISwitchProps
	extends React.ComponentProps<typeof SwitchPrimitiveRoot> {
	leftLabel?: string
	rightLabel?: string
}

export function Switch({
	className,
	leftLabel,
	rightLabel,
	...props
}: ISwitchProps) {
	return (
		<XStack className="gap-2 items-center">
			{leftLabel && (
				<Label data-slot="switch-left-label">{leftLabel}</Label>
			)}
			<SwitchPrimitiveRoot
				data-slot="switch"
				className={cn(
					"peer data-[state=checked]:bg-primary data-[state=unchecked]:bg-input focus-visible:border-ring focus-visible:ring-ring/50 dark:data-[state=unchecked]:bg-input/80 inline-flex h-[1.15rem] w-8 shrink-0 items-center rounded-full border border-transparent shadow-xs transition-all outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer",
					className,
				)}
				{...props}
			>
				<SwitchPrimitiveThumb
					data-slot="switch-thumb"
					className={cn(
						"bg-background dark:data-[state=unchecked]:bg-foreground dark:data-[state=checked]:bg-primary-foreground pointer-events-none block size-4 rounded-full ring-0 transition-transform data-[state=checked]:translate-x-[calc(100%-2px)] data-[state=unchecked]:translate-x-0",
					)}
				/>
			</SwitchPrimitiveRoot>
			{rightLabel && (
				<Label data-slot="switch-right-label">{rightLabel}</Label>
			)}
		</XStack>
	)
}
